...
class TrainDataset(Dataset):
    def __init__(self, queries, nentity, nrelation, negative_sample_size, answer, keep_entities=None):
        # queries is a list of (query, query_structure) pairs
        self.len = len(queries)
        self.queries = queries
        self.nentity = nentity
        ...
        if keep_entities is not None:
            self.inductive = True
            self.train_nodes = keep_entities[0].cpu().numpy()
        else:
    ...
    def __getitem__(self, idx):
        ...
        while negative_sample_size < self.negative_sample_size:
            if self.inductive:
                negative_sample = np.random.choice(self.train_nodes, size=self.negative_sample_size*2)
            else:
