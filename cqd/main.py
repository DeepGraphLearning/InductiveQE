...
from util import flatten_query, parse_time, set_global_seed, eval_tuple, load_file, get_unique_nodes, get_unique_nodes_wikikg
...
def parse_args(args=None):
    ...
    parser.add_argument('--skip_tr', action='store_true', default=False)
...
def evaluate(model, tp_answers, fn_answers, args, dataloader, query_name_dict, mode, step, writer):
    ...
    if args.inductive:
        if mode == 'Valid':
            keep_ents = model.val_nodes
            model.mode = mode
        elif mode == 'Test':
            keep_ents = model.test_nodes
            model.mode = mode
        else:
            keep_ents = model.train_nodes
            model.mode = mode
    else:
        keep_ents = None

    metrics = KGReasoning.test_step(model, tp_answers, fn_answers, args, dataloader, query_name_dict, keep_ents=keep_ents)
...
def load_data(args, tasks):
    ...
    if not args.inductive:
        ...
        #train_queries, train_answers = None, None
        ...
        train_hard_answers = train_answers
    else:
        if args.do_train:
            train_queries = pickle.load(open(os.path.join(args.data_path, "train_queries.pkl"), 'rb'))
            train_answers = pickle.load(open(os.path.join(args.data_path, "train_answers_hard.pkl"), 'rb'))
        else:
            train_queries, train_answers = None, None
        if args.do_valid:
            if not args.eval_train:
                valid_queries = pickle.load(open(os.path.join(args.data_path, "valid_queries.pkl"), 'rb'))
                valid_hard = pickle.load(open(os.path.join(args.data_path, "valid_answers_hard.pkl"), 'rb'))
                valid_easy = pickle.load(open(os.path.join(args.data_path, "valid_answers_easy.pkl"), 'rb'))
            else:
                valid_queries = pickle.load(open(os.path.join(args.data_path, "train_queries.pkl"), 'rb'))
                valid_hard = pickle.load(open(os.path.join(args.data_path, "train_answers_val.pkl"), 'rb'))
        else:
            valid_queries = None

        if not args.eval_train:
            test_queries = pickle.load(open(os.path.join(args.data_path, "test_queries.pkl"), 'rb'))
            test_hard = pickle.load(open(os.path.join(args.data_path, "test_answers_hard.pkl"), 'rb'))
            test_easy = pickle.load(open(os.path.join(args.data_path, "test_answers_easy.pkl"), 'rb'))
        else:
            train_answers = defaultdict(set)
            test_queries = pickle.load(open(os.path.join(args.data_path, "train_queries.pkl"), 'rb'))
            test_hard = pickle.load(open(os.path.join(args.data_path, "train_answers_test.pkl"), 'rb'))
            train_answers_hard = pickle.load(open(os.path.join(args.data_path, "train_answers_hard.pkl"), 'rb'))

        valid_hard_answers, valid_easy_answers, test_hard_answers, test_easy_answers = defaultdict(set), defaultdict(set), defaultdict(set), defaultdict(set)
        #if args.do_train:
        train_hard_answers = defaultdict(set)

        # load triples and extract valid_inference and test_inference nodes
        if "wikikg2" in args.data_path:
            train_graph = torch.load(os.path.join(args.data_path, "train_graph.pt"), map_location="cpu")
            val_inference = torch.load(os.path.join(args.data_path,"val_inference.pt"), map_location="cpu")
            test_inference = torch.load(os.path.join(args.data_path,"test_inference.pt"), map_location="cpu")
            train_nodes, val_nodes, test_nodes = get_unique_nodes_wikikg(train_graph, val_inference, test_inference)
        else:
            train_graph = load_file(os.path.join(args.data_path, "train_graph.txt"))
            val_inference = load_file(os.path.join(args.data_path, "val_inference.txt"))
            test_inference = load_file(os.path.join(args.data_path, "test_inference.txt"))

            val_nodes, test_nodes = get_unique_nodes(train_graph, val_inference, test_inference)
            train_nodes = torch.tensor(np.unique(np.array(train_graph)[:, [0, 1]]))
    ...
    for name in all_tasks:
        ...
        if name not in tasks or evaluate_union != args.evaluate_union:
            ...
            if not args.inductive or args.do_train:
            ...
            if args.inductive:
                ...
                if not args.eval_train:
                    if args.do_train:
                        train_hard_answers.update(train_answers[query_structure])
                    if args.do_valid:
                        valid_hard_answers.update(valid_hard[query_structure])
                        valid_easy_answers.update(valid_easy[query_structure])
                    test_hard_answers.update(test_hard[query_structure])
                    test_easy_answers.update(test_easy[query_structure])
                else:
                    if args.do_valid:
                        valid_hard_answers.update({
                            q: valid_hard[query_structure][i] for i, q in enumerate(test_queries[query_structure])
                        })
                    test_hard_answers.update(
                        {
                            q: test_hard[query_structure][i] for i, q in enumerate(test_queries[query_structure])
                        }
                    )
                    train_hard_answers.update({
                        q: train_answers_hard[query_structure][q] for q in test_queries[query_structure]
                    })

    return train_queries, train_hard_answers, valid_queries, valid_hard_answers, valid_easy_answers, test_queries, test_hard_answers, test_easy_answers, (train_nodes, val_nodes, test_nodes)
...
def main(args):
    ...
    if not args.inductive:
        ...
        if args.geo == "cqd":
            # TODO: WIP changed from args.data_path
            with open(f"{'/'.join(args.checkpoint_path.split('/')[:-1])}/e2id.pkl", 'rb') as f:
                e2id = pickle.load(f)
            with open(f"{'/'.join(args.checkpoint_path.split('/')[:-1])}/r2id.pkl", 'rb') as f:
                r2id = pickle.load(f)
            nentity = len(e2id)
            nrelation = len(r2id) + 1
        else:
            with open('%s/stats.txt' % args.data_path) as f:
                entrel = f.readlines()
                nrelation = int(entrel[1].split(':')[-1])
                train_ent = int(entrel[2].split(':')[-1])
                valid_ent = int(entrel[3].split(':')[-1])
                test_ent = int(entrel[4].split(':')[-1])
                nentity = train_ent + (valid_ent - train_ent) + (test_ent - train_ent) + 1
                nentity = int(entrel[0].split(':')[-1])
    ...
    train_queries, train_answers, valid_queries, valid_hard_answers, valid_easy_answers, test_queries, test_hard_answers, test_easy_answers, keep_entities = load_data(args, tasks)
    ...
    if args.do_train:
        ...
        train_path_iterator = SingledirectionalOneShotIterator(DataLoader(
                                    TrainDatasetClass(train_path_queries, nentity, nrelation, args.negative_sample_size, train_answers, keep_entities=keep_entities if args.inductive else None),
        ...
        if len(train_other_queries) > 0:
            ...
            train_other_iterator = SingledirectionalOneShotIterator(DataLoader(
                                        TrainDatasetClass(train_other_queries, nentity, nrelation, args.negative_sample_size, train_answers, keep_entities=keep_entities if args.inductive else None),
                                        batch_size=args.batch_size,
                                        shuffle=True,
                                        num_workers=args.cpu_num,
                                        collate_fn=TrainDatasetClass.collate_fn
    ...
    if args.geo == 'cqd':
        model = CQD(nentity,
                    ...
                    use_cuda=args.cuda,
                    e2id=e2id if args.inductive else None,
                    r2id=r2id if args.inductive else None,
    ...
    if args.checkpoint_path is not None:
        ...
        if args.geo != "cqd":
            ...
            checkpoint_ents = torch.load(
                f"{args.checkpoint_path}_ents.pt", map_location=torch.device('cpu') if not args.cuda else None
            )
            checkpoint_rels = torch.load(
                f"{args.checkpoint_path}_rels.pt", map_location=torch.device('cpu') if not args.cuda else None
            )
            model.load_weights(checkpoint_ents, checkpoint_rels)
            init_step = 0
    ...
    # eval on train
    if args.eval_train and not args.skip_tr:
        logging.info('Evaluating on Train Dataset...')
        train_easy_answers = defaultdict(set)
        train_all_metrics = evaluate(model, train_easy_answers, train_answers, args, test_dataloader, query_name_dict, 'Train', step, writer)
