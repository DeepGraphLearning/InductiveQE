...
def query_1p(entity_embeddings: nn.Module,
             ...
             ent_mapping: torch.LongTensor = None,
             rel_mapping: torch.LongTensor = None) -> Tensor:

    if ent_mapping is not None:
        queries[:, 0] = ent_mapping[queries[:, 0]]
        queries[:, 1] = rel_mapping[queries[:, 1]]
...
def query_2p(entity_embeddings: nn.Module,
    ...
    if ent_mapping is not None:
        queries[:, 0] = ent_mapping[queries[:, 0]]
        queries[:, 1] = rel_mapping[queries[:, 1]]
        queries[:, 2] = rel_mapping[queries[:, 2]]
...
def query_3p(entity_embeddings: nn.Module,
    ...
    if ent_mapping is not None:
        ...
        queries[:, 1] = rel_mapping[queries[:, 1]]
        queries[:, 2] = rel_mapping[queries[:, 2]]
        queries[:, 3] = rel_mapping[queries[:, 3]]
...
def query_2i(entity_embeddings: nn.Module,
    ...
    scores_1 = query_1p(entity_embeddings=entity_embeddings, predicate_embeddings=predicate_embeddings,
                        queries=queries[:, 0:2], scoring_function=scoring_function, ent_mapping=ent_mapping, rel_mapping=rel_mapping)
    scores_2 = query_1p(entity_embeddings=entity_embeddings, predicate_embeddings=predicate_embeddings,
                        queries=queries[:, 2:4], scoring_function=scoring_function, ent_mapping=ent_mapping, rel_mapping=rel_mapping)
...
def query_3i(entity_embeddings: nn.Module,
    ...
    scores_3 = query_1p(entity_embeddings=entity_embeddings, predicate_embeddings=predicate_embeddings,
                        queries=queries[:, 4:6], scoring_function=scoring_function, ent_mapping=ent_mapping, rel_mapping=rel_mapping)
...
def query_ip(entity_embeddings: nn.Module,
    ...
    scores_1 = query_2i(entity_embeddings=entity_embeddings, predicate_embeddings=predicate_embeddings,
                        queries=queries[:, 0:4], scoring_function=scoring_function, t_norm=t_norm, ent_mapping=ent_mapping, rel_mapping=rel_mapping)

    # [B, E]
    if ent_mapping is not None:
        queries[:, 4] = rel_mapping[queries[:, 4]]
...
def query_pi(entity_embeddings: nn.Module,
    ...
    scores_1 = query_2p(entity_embeddings=entity_embeddings, predicate_embeddings=predicate_embeddings,
                        queries=queries[:, 0:3], scoring_function=scoring_function, k=k, t_norm=t_norm, ent_mapping=ent_mapping, rel_mapping=rel_mapping)
    scores_2 = query_1p(entity_embeddings=entity_embeddings, predicate_embeddings=predicate_embeddings,
                        queries=queries[:, 3:5], scoring_function=scoring_function, ent_mapping=ent_mapping, rel_mapping=rel_mapping)
...
def query_2u_dnf(entity_embeddings: nn.Module,
                 ...
                 queries: Tensor,
                 scoring_function: Callable[[Tensor, Tensor, Tensor], Tensor],
                 t_conorm: Callable[[Tensor, Tensor], Tensor],
                 ent_mapping: torch.LongTensor = None,
                 rel_mapping: torch.LongTensor = None) -> Tensor:
...
def query_up_dnf(entity_embeddings: nn.Module,
    ...
    scores_1 = query_2u_dnf(entity_embeddings=entity_embeddings, predicate_embeddings=predicate_embeddings,
                            ...
                            ent_mapping=ent_mapping, rel_mapping=rel_mapping)

    # [B, E]
    if ent_mapping is not None:
        queries[:, 5] = rel_mapping[queries[:, 5]]
