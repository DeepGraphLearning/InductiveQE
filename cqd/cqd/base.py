...
from .discrete import query_ip, query_pi, query_3i, query_2i, query_3p, query_2p, query_1p, query_up_dnf, query_2u_dnf
...
class CQD(nn.Module):
    ...
    def __init__(self,
        ...
        if e2id is not None:
            local_eids, global_eids = torch.LongTensor(list(e2id.values())), torch.LongTensor([int(v) for v in list(e2id.keys())])
            self.entity_mapping = torch.full((global_eids.max()+1,), fill_value=-1, dtype=torch.long)
            self.entity_mapping[global_eids] = local_eids

            local_rids, global_rids = torch.LongTensor(list(r2id.values())), torch.LongTensor([int(v) for v in list(r2id.keys())])
            self.rel_mapping = torch.full((global_rids.max()+1, ), fill_value=-1, dtype=torch.long)
            self.rel_mapping[global_rids] = local_rids

            if self.use_cuda:
                self.entity_mapping = self.entity_mapping.cuda()
                self.rel_mapping = self.rel_mapping.cuda()

        self.inductive = inductive

        if keep_entities is not None:
            train_nodes, val_nodes, test_nodes = keep_entities[0], keep_entities[1], keep_entities[2]

            # remap
            self.train_nodes = self.entity_mapping[train_nodes.to(self.entity_mapping.device)]
            self.val_nodes = self.entity_mapping[val_nodes.to(self.entity_mapping.device)]
            self.test_nodes = self.entity_mapping[test_nodes.to(self.entity_mapping.device)]

            self.train_mask = torch.zeros(self.embeddings[0].weight.data.shape[0], dtype=torch.bool,
                                        device=self.entity_mapping.device)
            self.train_mask[self.train_nodes] = 1

            self.val_mask = torch.zeros(self.embeddings[0].weight.data.shape[0], dtype=torch.bool, device=self.entity_mapping.device)
            self.val_mask[self.val_nodes] = 1

            self.test_mask = torch.zeros(self.embeddings[0].weight.data.shape[0], dtype=torch.bool,
                                        device=self.entity_mapping.device)
            self.test_mask[self.test_nodes] = 1
    ...
    def score_candidates(self,
        ...
        if not self.inductive:
            ...
            lhs_emb = self.embeddings[0](self.entity_mapping[triples[:, 0]])
            rel_emb = self.embeddings[1](self.rel_mapping[triples[:, 1]])
            rhs_emb = self.embeddings[0](self.entity_mapping[triples[:, 2]])
    ...
    def forward(self,
        ...
        for query_structure, queries in batch_queries_dict.items():
            ...
            # remapping for inductive cases
            if self.inductive:
                head = self.entity_mapping[head]
                rel = self.rel_mapping[rel]
            ...
            if 'continuous' in self.method:
                ...
                def scoring_function(rel_: Tensor, lhs_: Tensor, rhs_: Tensor) -> Tensor:
                    ...
                    if self.mode == "Valid":
                        # mask scores of nodes not in valid_inference
                        res[:, ~self.val_mask] = float('-inf')
                    if self.mode == "Test":
                        # mask scores of nodes not in test_inference
                        res[:, ~self.test_mask] = float('-inf')
                    if self.mode == "Train":
                        res[:, ~self.train_mask] = float('-inf')
                ...
                if graph_type == "1p":
                    scores = query_1p(entity_embeddings=self.embeddings[0],
                                         predicate_embeddings=self.embeddings[1],
                                         queries=queries,
                                         scoring_function=scoring_function,
                                         ent_mapping=self.entity_mapping if self.inductive else None,
                                         rel_mapping=self.rel_mapping if self.inductive else None)
                elif graph_type == "2p":
                    scores = query_2p(entity_embeddings=self.embeddings[0],
                                         predicate_embeddings=self.embeddings[1],
                                         queries=queries,
                                         scoring_function=scoring_function,
                    ...
                    scores = query_3p(entity_embeddings=self.embeddings[0],
                                         predicate_embeddings=self.embeddings[1],
                                         queries=queries,
                                         scoring_function=scoring_function,
                                         k=self.k, t_norm=t_norm,
                    ...
                    scores = query_2i(entity_embeddings=self.embeddings[0],
                                         predicate_embeddings=self.embeddings[1],
                                         queries=queries,
                    ...
                    scores = query_3i(entity_embeddings=self.embeddings[0],
                                         predicate_embeddings=self.embeddings[1],
                                         queries=queries,
                                         scoring_function=scoring_function, t_norm=t_norm,
                                         ent_mapping=self.entity_mapping if self.inductive else None,
                    ...
                    scores = query_pi(entity_embeddings=self.embeddings[0],
                                         predicate_embeddings=self.embeddings[1],
                                         queries=queries,
                                         scoring_function=scoring_function,
                                         k=self.k, t_norm=t_norm,
                    ...
                    scores = query_ip(entity_embeddings=self.embeddings[0],
                                         predicate_embeddings=self.embeddings[1],
                                         queries=queries,
                                         scoring_function=scoring_function,
                                         k=self.k, t_norm=t_norm,
                    ...
                    scores = query_2u_dnf(entity_embeddings=self.embeddings[0],
                                             predicate_embeddings=self.embeddings[1],
                                             queries=queries,
                                             scoring_function=scoring_function,
                                             t_conorm=t_conorm,
                    ...
                    scores = query_up_dnf(entity_embeddings=self.embeddings[0],
                                             predicate_embeddings=self.embeddings[1],
                                             queries=queries,
                                             scoring_function=scoring_function,
    ...
    def load_weights(self, ents, rels):
        self.embeddings[0].weight.data = ents
        self.embeddings[1].weight.data = rels

        self.original_ent_embeddings = self.embeddings[0].weight.data.clone()


        print("Initialized")

    def set_entity_embedding(self, keep_ents):

        mask = torch.zeros(self.embeddings[0].weight.data.shape[0], dtype=torch.bool, device=self.embeddings[0].weight.data.device)
        mask[keep_ents.to(mask.device)] = 1

        all_embs = self.original_ent_embeddings.clone().to(self.embeddings[0].weight.data.device)
        all_embs[~mask] = float('-inf')
        self.embeddings[0].weight.data = all_embs
        print(f"Set entity embeddings to have {len(keep_ents)} non-zero elements")