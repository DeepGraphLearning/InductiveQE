...
class KGReasoning(nn.Module):
    def __init__(self, nentity, nrelation, hidden_dim, gamma, 
                 ...
                 inductive=False, keep_entities=None,):
        ...
        if keep_entities is not None:
            self.register_buffer("train_nodes", keep_entities[0])
            self.register_buffer("val_nodes", keep_entities[1])
            self.register_buffer("test_nodes", keep_entities[2])

            train_mask = torch.zeros(nentity, dtype=torch.bool)
            train_mask[self.train_nodes] = 1

            val_mask = torch.zeros(nentity, dtype=torch.bool)
            val_mask[self.val_nodes] = 1

            test_mask = torch.zeros(nentity, dtype=torch.bool)
            test_mask[self.test_nodes] = 1
    ...
    @staticmethod
    def test_step(model, easy_answers, hard_answers, args, test_dataloader, query_name_dict, save_result=False, save_str="", save_empty=False, keep_ents=None):
        model.eval()
        ...
        with torch.set_grad_enabled(requires_grad):
            for negative_sample, queries, queries_unflatten, query_structures in tqdm(test_dataloader, disable=not args.print_on_screen):
                ...
                # keep scores of only existing nodes in the current graph, set others to -inf
                if keep_ents is not None:
                    mask = torch.zeros(negative_logit.shape[-1], dtype=torch.bool, device=negative_logit.device)
                    mask[keep_ents.to(negative_logit.device)] = 1
                    negative_logit[:, ~mask] = float('-inf')
                ...
                for idx, (i, query, query_structure) in enumerate(zip(argsort[:, 0], queries_unflatten, query_structures)):
                    ...
                    if model.inductive:
                        ...
                        if geo is None:
                            hard_answer = set(model.entity_mapping[torch.LongTensor(list(hard_answer)).to(model.entity_mapping.device)].cpu().numpy().tolist())
                            if num_easy > 0:
                                easy_answer = set(model.entity_mapping[torch.LongTensor(list(easy_answer)).to(model.entity_mapping.device)].cpu().numpy().tolist())
                        else:
                            hard_answer = set(hard_answer)
                            easy_answer = set(easy_answer)
                    ...
                    roc_score = roc_auc_score(masks.cpu().float(), cur_ranking.cpu()) if num_easy > 0 else -1

                    cur_ranking = cur_ranking - answer_list + 1 # filtered setting

                    rank_easy = cur_ranking[~masks].mean().item() if num_easy > 0 else -1.0
                    mrr_easy = torch.mean(1./cur_ranking[~masks]).item() if num_easy > 0 else -1.0
                    rank_hard = cur_ranking[masks].mean().item()
                    mrr_hard = torch.mean(1./cur_ranking[masks]).item()
                    ...
                    h100 = torch.mean((cur_ranking <= 100).to(torch.float)).item()
                    ...
                    logs[query_structure].append({
                        ...
                        'avg_easy_rank': rank_easy,
                        'avg_hard_rank': rank_hard,
                        'avg_easy_mrr': mrr_easy,
                        'avg_hard_mrr': mrr_hard,
                        'avg_rocauc': roc_score,
                    })
        ...
        for query_structure in logs:
            for metric in logs[query_structure][0].keys():
                if metric in ['num_hard_answer']:
                    continue
                if 'avg' in metric:
                    nonzero_metrics = [log[metric] for log in logs[query_structure] if log[metric] > -1]
                    # for eval on training data with possible -1's
                    if len(nonzero_metrics) == 0:
                        metrics[query_structure][metric] = -1
                    else:
                        metrics[query_structure][metric] = sum(nonzero_metrics) / len(nonzero_metrics)
                else:
