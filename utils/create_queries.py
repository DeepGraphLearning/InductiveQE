# taken from https://github.com/snap-stanford/KGReasoning/blob/main/create_queries.py
...
def set_logger(save_path, query_name, print_on_screen=True):
    '''
...
def write_links(dataset, ent_out, small_ent_out, max_ans_num, name):
    ...
    :param ent_out: current set of entities in validation/test triples
    :param small_ent_out: a bigger set of entities in validation_inference + train
    :param max_ans_num: const, about 1000
    :return:
    """
    ...
    for ent in ent_out:
        for rel in ent_out[ent]:
            if len(ent_out[ent][rel]) <= max_ans_num:
                ...
                # # true positives - those appearing in the val/test triples to predict
                # tp_answers[(ent, (rel,))] = ent_out[ent][rel]
                #
                # # false negatives - the triples that already exist in the validation_inference + train
                # fn_answers[(ent, (rel,))] = small_ent_out[ent][rel]
...
def generate_queries(dataset, query_structures, gen_num, max_ans_num, gen_train, gen_valid, gen_test, query_names,
                     save_name):
    # now dataset is a list of [train_triples, valid_triples, test_triples]
    # base_path = './data/%s' % dataset
    # indexified_files = ['train_indexified.txt', 'valid_indexified.txt', 'test_indexified.txt']
    train_triples, valid_triples, test_triples = dataset
    #
    if gen_train or gen_valid:
        #train_ent_in, train_ent_out = construct_graph(base_path, indexified_files[:1])  # ent_in

        # Training graph contains training triples
        train_ent_in, train_ent_out = construct_graph_from_triples(train_triples)  # ent_in
    if gen_valid or gen_test:
        ...
        # This graph contains triples from validation (unseen nodes) + train (seen nodes) , a union of two graphs
        valid_ent_in, valid_ent_out = construct_graph_from_triples(np.concatenate([train_triples, valid_triples[0]], axis=0).tolist())

        # This graph contains triples only from validation (unseen to unseen links and some unseen to seen links)
        valid_only_ent_in, valid_only_ent_out = construct_graph_from_triples(valid_triples[1])

        # full valid graph + train including inference and predicted edges
        full_val_in, full_val_out = construct_graph_from_triples(
            np.concatenate([
                train_triples, valid_triples[0], valid_triples[1]
            ], axis=0).tolist()
        )
    if gen_test:
        ...
        # This is a union of test and train graphs
        test_ent_in, test_ent_out = construct_graph_from_triples(np.concatenate([train_triples, test_triples[0]], axis=0).tolist())

        # This is a test graph only with unseen to unseen links and few unseen to seen links
        test_only_ent_in, test_only_ent_out = construct_graph_from_triples(test_triples[1])

        # full test
        full_test_in, full_test_out = construct_graph_from_triples(
            np.concatenate([
                train_triples, test_triples[0], test_triples[1]
            ], axis=0).tolist()
        )
    ...
    if query_structure == ['e', ['r']]:
        if gen_train:
            q, tp, fn, fp = write_links(dataset, train_ent_out, defaultdict(lambda: defaultdict(set)), max_ans_num,
                        'train-' + query_name)
            train_queries.update(q)
            train_tp_answers.update(tp)
            train_fn_answers.update(fn)
        if gen_valid:
            # q, tp, fn, fp = write_links(dataset, valid_only_ent_out, train_ent_out, max_ans_num, 'valid-' + query_name)
            q, tp, fn, fp = write_links(dataset, valid_only_ent_out, valid_ent_out, max_ans_num, 'valid-' + query_name)
            valid_queries.update(q)
            valid_tp_answers.update(tp)
            valid_fn_answers.update(fn)
        if gen_test:
            # q, tp, fn, fp = write_links(dataset, test_only_ent_out, valid_ent_out, max_ans_num, 'test-' + query_name)
            q, tp, fn, fp = write_links(dataset, test_only_ent_out, test_ent_out, max_ans_num, 'test-' + query_name)
            test_queries.update(q)
            test_tp_answers.update(tp)
            test_fn_answers.update(fn)
    ...
    if gen_train:
        train_queries, train_tp_answers, train_fp_answers, train_fn_answers = ground_queries(dataset, query_structure,
                                                                                             train_ent_in,
                                                                                             train_ent_out, defaultdict(
                lambda: defaultdict(set)), defaultdict(lambda: defaultdict(set)),
                                                                                             gen_num[0], max_ans_num,
                                                                                             query_name, 'train',
                                                                                             ent2id, rel2id)
    if gen_valid:
        # before: valid_ent_in, valid_ent_out, train_ent_in, train_ent_out
        valid_queries, valid_tp_answers, valid_fp_answers, valid_fn_answers = ground_queries(dataset, query_structure,
                                                                                             full_val_in,
                                                                                             full_val_out,
                                                                                             valid_ent_in,  # default: train_ent_in and out
                                                                                             valid_ent_out, gen_num[1],
...
def achieve_answer(query, ent_in, ent_out):
    ...
    if all_relation_flag:
        ...
        for i in range(len(query[-1])):
            if query[-1][i] == -2:
                # BUGFIX here, old line commented out
                # Our universe of entities comes from the keys of the array, as IDs might be non-contiguous
                # and then range(len()) will produce incorrect results
                ent_set = set(list(ent_in.keys())) - ent_set
